...
if (!class_exists('asthir_About')) {
	...
	class asthir_About
	{
		 ...
		 *
		 * @var asthir_About $instance The  TI_About_Page instance.
		 */
		...
		{
			if (!isset(self::$instance) && !(self::$instance instanceof asthir_About)) {
				self::$instance = new asthir_About;
		...
		public function ti_about_page_render()
		{
			...
			if (!empty($welcome_title) || !empty($welcome_content) || !empty($this->tabs)) {
				...
				echo '<div class="wrap about-wrap asthir-about-wrap">';
...
?>
			<div class="feature-section asthir-section asthir-section-getting-started three-col">
				<?php foreach ($content as $item) : ?>
					<?php $this->render_grid_item($item); ?>
				<?php endforeach; ?>
			</div><!-- .feature-section .asthir-section -->
		<?php
		}
		...
		?>
			<div class="feature-section asthir-section asthir-section-support three-col">
		...
		public function free_pro()
		{
			...
			if (!empty($free_pro)) {
				if (!empty($free_pro['free_theme_name']) && !empty($free_pro['pro_theme_name']) && !empty($free_pro['features']) && is_array($free_pro['features'])) {
					...
					echo '<tr><td style="border-top:0;"></td><td style="border-top:0;"></td><td style="border-top:0;" class="only-pro"><a href="' . esc_url('https://wpthemespace.com/product/asthir-pro') . '" target="_blank" >';
					echo esc_html('Asthir Pro Live Preview', 'asthir');
					echo '</a></td></tr>';
					if (!empty($free_pro['pro_theme_link']) && !empty($free_pro['get_pro_theme_label'])) {
						echo '<tr class="about-page-text-center">';
						echo '<td></td>';
						echo '<td colspan="2"><a href="' . esc_url('https://wpthemespace.com/product/asthir-pro/?add-to-cart=3508') . '" target="_blank" class="button button-primary button-hero">' . wp_kses_post($free_pro['get_pro_theme_label']) . '</a></td>';
						echo '</tr>';
					}
					echo '</tbody>';
					echo '</table>';
			...
			if ('appearance_page_' . $this->theme_slug . '-welcome' == $hook_suffix) {
				...
				wp_localize_script(
					...
					array(
						...
						'activating_string'        => __('Activating', 'asthir')
					)
				);
			}
		}
